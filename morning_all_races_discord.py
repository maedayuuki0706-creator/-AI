...
def sent_morning_keys(day: str) -> set[tuple[str, int]]:
    ...
    for line in base.LOG_PATH.read_text(encoding="utf-8").splitlines():
        try:
            ...
            continue
        if row.get("day") == day and row.get("phase") == "morning":
            try:
...
def compact_formation(rows: list[dict]) -> tuple[str, dict]:
    ...
    for section in summary["sections"]:
        ...
        parts.append(f"{labels[section['key']]}:{forms}")
...
def compact_virtual(allocation: dict) -> str:
    ...
    return f"仮{allocation['total_units']}口 {bets}→最低{allocation['min_return_units']:.1f}口"


def send_chunks(header: str, race_lines: list[str]) -> int:
    chunks = []
    current = header
...
def run_once(now: datetime | None = None) -> int:
    ...
    first_message = (
        f"🌅 **競艇AIナビ｜全レース朝予想＋仮想投票 {day[4:6]}/{day[6:8]}**\n"
        f"対象 {len(venues)}会場 / 今回 {len(analyses)}R / オッズ取得 {coverage:.0%}\n"
        "1口=100円換算。公開オッズでガミ回避できない組み合わせは0口見送り。"
    )
    if passed:
        first_message += f"\n※本日は復旧送信のため締切済み{passed}Rは仮想投票対象外。"
    ...
    for jcd in venues:
        ...
        for rno, deadline, analysis in races:
            ...
            records.append((rno, deadline, analysis, rows, summary, allocation, rank, category))

        if not lines:
            continue
        header = f"🚤 **{base.VENUES.get(jcd, jcd)}｜朝の全R予想**　仮想合計 {venue_units}口"
        try:
            send_chunks(header, lines)
        except Exception as exc:
            print(f"morning Discord failed {jcd}: {type(exc).__name__}")
            continue

        sent_at = datetime.now(base.JST).isoformat()
        for rno, deadline, analysis, rows, summary, allocation, rank, category in records:
            combos = [row["combination"] for row in rows]
            base.log_prediction({
                "day": day,
                "jcd": jcd,
